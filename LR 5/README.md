...
## Пояснение к коду
### Функция fact_recursive
Рекурсивная функция нахождения факториала. Если значение n будет меньше нуля, то вернется строка "Число не должно быть меньше нуля". Если n = 0 или 1 то вернется 1. Во всех остальных случаях будет возвращаться значение n * fact_recursive(n - 1).
### Функция fact_iterative
Итеративная функция нахождения факториала. Если значение n будет меньше нуля, то вернется строка "Число не должно быть меньше нуля". После чего сразу задается значение result = 1. Дальше создается цикл for от 1 до n + 1, в котором результат умножается на текущее значение i. В конце возвращается result.
### Функции fact_recursive_memo и fact_iterative_memo
Те же самые функции, но которые используют мемоизацию с помощью lru_cache. При повторных вызовах будут возвращать результаты из кэша.
### Функция benchmark
Аргументы:
- func - тестируемая функция
- n - аргумент для тестируемой функции
- number - количество вызовов за раз
- repeat - количество повторений

В переменную times записывается время выполнения функции. С помощью timeit.repeat это происходит несколько раз для более точного измерения. В конце возвращается минимальный результат, который получился.
### Функция main
Создается список значений n_list от 10 до 300 с шагом 10. Также создаются списки для записи времени каждой функции. В цикле for для n, который находится в диапазоне n_list проводится бенчмаркинг всех функций, после чего добавляются получившиеся значения в ранее созданные списки.

С помощью библиотеки matplotlib строятся графики, где ось х - это число n, а ось y - это время, за которое было вычислено n.
